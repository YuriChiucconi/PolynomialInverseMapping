...
PIM<dim>::initialize(const Triangulation<dim> & tria)
{
    ...
    for (const auto &cell : inverse_dh->active_cell_iterators())
    {
        ...
        for (auto i : iota(0, dof_indices.size()))
            {
                const auto d = inverse_fe_system.system_to_component_index(i).first;
                const auto j = inverse_fe_system.system_to_component_index(i).second;
                                           ...
                                           , const Point<dim> & p) const
{
    ...
    Point<dim> q = box.real_to_unit(p);
    return inverse_mapping->transform_unit_to_real_cell(cell, q);
}
